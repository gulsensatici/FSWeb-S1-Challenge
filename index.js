...
function indekseGoreFenomen(/*kod*/fenomen, fenomenIndex) {
 let ıstenilen = fenomenIndex + ". indekste bulunan fenomen: " + fenomen[fenomenIndex].profile;
 return ıstenilen;
  /*kod*/
}
...
function profilListesi(/*kod*/fenomenler) {
  let yeni= [];
  for(let i=0; i<fenomenler.length; i++){
    yeni.push(fenomenler[i].profile);
  }
return yeni;
}
...
function fenomenSil(/*kod*/fenomenlerinDizisi, diziIndex) {
  let sonrakiDizi= [...fenomenlerinDizisi];
 sonrakiDizi.splice(diziIndex,1);
 return sonrakiDizi;
...
function fenomenEkle(/*kod*/fenomenler,number,profile,followers,post,platform) {
let ekle =[...fenomenler];
  let yeniDizi=
    {
      "number": number,
...
function enFenomenler(/*kod*/fenomenler) {
  let dizi=[];
  for(let i=0; i<fenomenler.length; i++){
...
function fenomenGonderimSayisi(/*kod*/fenomenlerDizisi, fenomenAdi){
  let dizi=fenomenlerDizisi.find(dizi => dizi.profile === fenomenAdi)
